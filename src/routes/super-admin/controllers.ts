...
import superAdminData from '../../data/super-admin';

const getAllSuperAdmin = (req: Request, res: Response) => {
  try {
    const allSuperAdmins = superAdminData.filter(
      (superAdmin) =>
        (superAdmin.firstName && superAdmin.firstName === req.body.firstName) ||
        superAdmin.isActive,
    );
    if (allSuperAdmins.length > 0) {
      return res.status(200).json({
        message: 'Showing SuperAdmin.',
        data: allSuperAdmins,
        error: false,
      });
    }
